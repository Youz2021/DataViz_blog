...
gps_path_selected <- gps_path2 %>%
  filter(day %in% c(12),hour %in% c(13))
...
According to the card transactions and gps data of cars, we can find that there were three employees used their credit cards at 'Chostus Hotel' and we can identify their car id using GPS data. Then we can link credit card to the owner of the car.
...
1. Some executives might go and play golf together in the afternoon of 12 and 19 January.

![](img/ceo.png){width=40%}
