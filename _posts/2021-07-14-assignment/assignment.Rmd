...
1. Change variables to the proper data type and create new variables for table merge and further analysis.
...
for the time of popularity

```{r}
popular_time <- joint_cc_loyalty %>%
  select(location,date,timestamp.x,price,weekday) %>%
  filter(location == "Katerina’s Café"|location == "Guy's Gyros"|location == "Hippokampos"|location == "Abila Airport"|location == "Nationwide Refinery"|location == "Carlyle Chemical Inc.") 
...
popular_time_count <- popular_time_count %>%
  ...
  replace_na(list(row_count = 0L)) %>% 
  filter(location == "Katerina’s Café")
``` 

```{r}
ggplot(popular_time_count,aes(date, hour, fill= row_count)) + 
geom_tile()
```
