...
geom_tile(color = 'white',size = 0.1) + 
  ...
  #scale_x_discr(breaks = popular_time_count$date[seq(1, length(popular_time_count$date), by = 2)])+
  scale_y_discrete()+
...
3. Transactions in some locations are higher than regular in some days.
...
ggplot(location_date,aes(date, location, fill= frequency)) + 
geom_tile(color = 'white',size = 0.1) + 
  scale_fill_distiller(palette = "orange",direction = 1)+
  scale_x_date(breaks = new_cc_loyalty_data$date[seq(1, length(new_cc_loyalty_data$date), by = 2)])+
...
1. PLot the track events for each car on the map to identify their activities.
...
## 4.7 Mapping credit card and loyalty card
...
## Q1. Using just the credit and loyalty card data, identify the most popular locations, and when they are popular. What anomalies do you see? What corrections would you recommend to correct these anomalies? 
...
2. There are just 17 credit cards which were used frequently at Brew've Been Served on weekdays around 7am to 9am, far less than that of other three popular locations. For Brew've Been Served anomaly, we can follow up the employees who owned these credit cards to identify their relationships.
...
4. The price of some transactions varies largely in the same location. For example, there is a transaction higher than others in 'Frydos Autosupply n' More'. We need to explore the anomalous transactions.

![](img/price_vary.png){width=90%}

5. Transactions in some locations are higher than regular in some days. For example, there were more transactions at Katerina’s Café on 8,11 and 18 January. Also, there were more transactions at Brew've Been Served on 8, 14 and 15 January. We need to explore the actives on that days.

![](img/popular_location_date.png){width=90%}


## Q2: Add the vehicle data to your analysis of the credit and loyalty card data. How does your assessment of the anomalies in question 1 change based on this new data? What discrepancies between vehicle, credit, and loyalty card data do you find?
