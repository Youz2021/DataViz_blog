...
Click 'Analysis' and select 'Create Calculated Field', then input 'Export' as the name of this field and input the formula in the box below. Repeat the same step to create the field called Import using 'Import' to replace 'Export' in the formula.

![](img/create-field.png){width=25%}
![](img/export-formula.png){width=50%}
...
Drag [Markets] to Detail,then the chart would show the detail for each Market.

![](img/detail.png){width=60%}

1.4 Change table calculation to percentile

Click the triangle sign of 'SUM(Import)' and 'SUM(Export)' and select 'Quick Table Calculation' and 'Percentile'. 

![](img/percentile.png){width=40%}

1.5 Change Compute Using

Click the triangle sign of 'SUM(Import)' and 'SUM(Export)' and select 'Compute Using' and 'Markets'.

![](img/compute.png){width=40%}

1.6 Apply color

Drag [Export] to color,then click color and edit color. Select 'Green-Blue Diverging' for palette and the graph will be colored automatically.
